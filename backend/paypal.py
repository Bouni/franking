...
class PayPal:
    ...
    def fetch_transactions(self, days: int = 30):
        with httpx.Client(base_url="https://api-m.paypal.com") as client:
            ...
            response = client.get(
                "/v1/reporting/transactions", headers=headers, params=params
            )
            return self.filter_transactions(
                response.json().get("transaction_details", [])
            )
                ...
                transaction_data.append(
                    {
                        "invoice": self.get_invoice_number(
                            t.get("transaction_info", {}).get("transaction_note", "")
                        ),
                        "name": t.get("payer_info", {})
                        .get("payer_name", {})
                        .get("alternate_full_name", ""),
                        "date": t.get("transaction_info", {}).get(
                            "transaction_updated_date", ""
                        ),
                        "amount": t.get("transaction_info", {})
